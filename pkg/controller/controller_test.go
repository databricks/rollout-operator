...
func TestRolloutController_Reconcile(t *testing.T) {
	...
	for testName, testData := range tests {
		t.Run(testName, func(t *testing.T) {
			...
			c := NewRolloutController(kubeClient, restMapper, scaleClient, dynamicClient, testNamespace, nil, 0, false, reg, log.NewNopLogger())
...
func TestRolloutController_ReconcileStatefulsetWithDownscaleBoolean(t *testing.T) {
	...
	for testName, testData := range tests {
		t.Run(testName, func(t *testing.T) {
			...
			// Create the controller and start informers.
			reg := prometheus.NewPedanticRegistry()
			c := NewRolloutController(kubeClient, restMapper, scaleClient, dynamicClient, testNamespace, httpClient, 0, false, reg, log.NewNopLogger())
			require.NoError(t, c.Init())
			defer c.Stop()
...
func TestRolloutController_ReconcileShouldDeleteMetricsForDecommissionedRolloutGroups(t *testing.T) {
	...
	// Create the controller and start informers.
	reg := prometheus.NewPedanticRegistry()
	c := NewRolloutController(kubeClient, nil, nil, nil, testNamespace, nil, 0, false, reg, log.NewNopLogger())
	require.NoError(t, c.Init())
	defer c.Stop()
